...
fn instr_list(rom_bytes: &[u8], region: &RomRegion) -> Vec<rabbitizer::Instruction> {
    let region_bytes = &rom_bytes[region.rom_start()..region.rom_end()];
    region_bytes
        .chunks_exact(INSTRUCTION_SIZE)
        .map(|v| new_instruction_cpu(read_be_word(v)))
        .collect()
}

fn summary(instrs: &[rabbitizer::Instruction], n: usize) -> DashMap<Vec<rabbitizer::InstrId>, usize, MyHasher> {
